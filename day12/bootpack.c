...
void HariMain(void)
{
	...
	unsigned char s[40], keybuf[32], mousebuf[128], timerbuf1[8], timerbuf2[8], timerbuf3[8];
	int mx, my, i;
		...
		if (fifo8_status(&keyfifo) + fifo8_status(&mousefifo) + fifo8_status(&timerfifo1) + fifo8_status(&timerfifo2) + fifo8_status(&timerfifo3) == 0) {
			...
			if (fifo8_status(&keyfifo) != 0) {
				...
				if (mouse_decode(&mdec, i) != 0) {
					...
					boxfill8(buf_back, binfo->scrnx, COL8_008484, 0, 0, 79, 15); /* 隐藏 */
					putfonts8_asc(buf_back, binfo->scrnx, 0, 0, COL8_FFFFFF, s); /* 画坐标 */
