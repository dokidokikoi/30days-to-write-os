...
void sheet_updown(struct SHEET *sht, int height)
{
	...
	if (old > height) {	/* 比以前低 */
		if (height >= 0) {
			...
			sheet_refreshsub(ctl, sht->vx0, sht->vy0, sht->vx0 + sht->bxsize, sht->vy0 + sht->bysize, sht->height+1, old); /* 按新图层的信息重新绘制画面 */
		} else {	/*隐藏*/
			if (ctl->top > old) {
				/* 把上面的降下来 */
		...
		sheet_refreshsub(ctl, sht->vx0, sht->vy0, sht->vx0 + sht->bxsize, sht->vy0 + sht->bysize, sht->height, height); /* 按新图层信息重新绘制画面 */
	}
	return;
}
...
void sheet_refreshmap(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0)
{
	...
	for (h = h0; h <= ctl->top; h++) {
		...
		if (sht->col_inv == -1) {
			if ((sht->vx0 & 3) == 0 && (bx0 & 3) == 0 && (bx1 & 3) == 0) { // 判断能否被4整除
				bx1 = (bx1 - bx0) / 4; /* MOV回数 */
...
void sheet_slide(struct SHEET *sht, int vx0, int vy0)
{
	...
	if (sht->height >= 0) { /* 如果正在显示*/
		...
		sheet_refreshsub(ctl, old_vx0, old_vy0, old_vx0 + sht->bxsize, old_vy0 + sht->bysize, 0, sht->height-1); /* 其它图层恢复移动图层移动前被遮盖的画面 */
